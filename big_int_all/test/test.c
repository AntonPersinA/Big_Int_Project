...
int test_miller_rabin(int limit) //Следует выставлять хотя бы 20
{
    for (int i = 0; i < limit; ++i)
    {
        ...
        char pr_test = 1;
        char pr_miller = big_int_miller_rabin(n1, 10 + (limit > 100 ? 100 : 1));
        big_int *zero = big_int_get("0");
        big_int *one = big_int_get("1");
...
int test_GCD(int limit)
{
    for (int num1 = 1; num1 <= limit; ++num1)
    {
        for (int num2 = 3; num2 <= limit + 3; ++num2)
        {
            big_int *n1 = big_int_get10((num1*limit) % 1001 + 1);
            big_int *n2 = big_int_get10((num2*limit) % 701 + 1);
            big_int *test_res = big_int_GCD(n1, n2);

            big_int *gcd = big_int_get("1");
            big_int *max_gcd = big_int_get("1");
            big_int *one = big_int_get("1");
            for ( ; (big_int_leq(gcd, n1)); )
            {
                big_int *md1 = big_int_mod(n1, gcd);
                big_int *md2 = big_int_mod(n2, gcd);
                if (big_int_equal_sgn(md1, md2) && md1->number[0] == 0 && md1->length == 1)
                {
                    big_int_free(&max_gcd);
                    max_gcd = big_int_copy(gcd);
                }
                big_int_add2(gcd, one);
                big_int_free2(2, &md1, &md2);
            }

            if (!(big_int_equal_sgn(test_res, max_gcd)))
            {
                printf("error test_GCD\n");
                printf("num1 = %d, num2 = %d\n", (num1*limit) % 1001 + 1, (num2*limit) % 701 + 1);
                big_int_print10(test_res);
                big_int_print10(max_gcd);

                big_int_free2(6, &n1, &n2, &gcd, &max_gcd, &one, &test_res);
                return 0;
            }
            big_int_free2(6, &n1, &n2, &gcd, &max_gcd, &one, &test_res);
        }
...
int test_reverse_in_ring(int limit)
{
    for (int num = 2; num <= limit + 1; ++num)
    {
        big_int *n1 = big_int_get10(num);
        big_int *md;
        if (num % 3 == 0)
        {
            md = big_int_rnd(n1->length + 1);
        }
        if (num % 3 == 1)
        {
            md = big_int_rnd_odd(n1->length + 1);
        }
        if (num % 3 == 2)
        {
            md = big_int_get_prime(n1->length + 1, 20);
        }
        big_int *tst = big_int_reverse_in_ring(n1, md);
        big_int *mul = big_int_karatsuba_mult2(tst, n1);
        big_int *res = big_int_mod(mul, md);

        big_int *one = big_int_get("1");
        big_int *gcd = big_int_GCD(n1, md);
        if (!(res->number[0] == 1 && res->length == 1 ) && (big_int_leq(gcd, one)))
        {
            printf("error test_GCD\n");
            printf("num = %d\n", num);
            printf("md = ");
            ...
            big_int_free2(7, &n1, &md, &tst, &mul, &res, &one, &gcd);
            return 0;
        }
        big_int_free2(7, &n1, &md, &tst, &mul, &res, &one, &gcd);
    }
